...
| chart_pop | trend/cost/scale/growth/decline claims ("costs explode", "can't scale", "left behind", "grows fast") | payload.chart {kind: bar_up\|bar_down\|line_up\|line_down, label ≤18 chars naming the thing, e.g. "COSTS", "SCALE", "YOUR VALUE"} — synthetic animated chart, no data needed |
...
13. **RICHNESS QUOTA (hard — a validator counts):** at least 40% of all beats must carry a visual element (an asset, or one of: stat_slam / list_stack / tweet_card / screenshot_zoom / logo_versus / emoji_burst / quote_card / chart_pop). Rule 12 restricts WHICH visuals abstract beats may use — it does NOT mean "make everything kinetic_type". Abstract/commentary content gets its richness from **chart_pop** (any claim about things growing/shrinking/costing/scaling — these are everywhere in opinion content), **quote_card** (the thesis line, attribution = the speaker or "The take"), **stat_slam** (any spoken number), **list_stack** (any enumeration), **emoji_burst** (the emotion/metaphor: 🚀 growth, 💸 cost, ⚠️ warning, 🤖 AI/agents, 🧠 thinking, 🔥 strong claim). A manifest of mostly-bare kinetic_type beats WILL BE REJECTED. b01 (hook) and the final beat MUST be visual.
13b. **VARIETY (hard — a validator counts):** emoji_burst may carry AT MOST HALF of the visual beats. Spread the rest across chart_pop / quote_card / stat_slam / screenshot_zoom / list_stack / cutout_pop. Named product/site/app spoken → screenshot_zoom or logo. Two named things compared → logo_versus.
