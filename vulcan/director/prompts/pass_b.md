...
| chart_pop | trend/cost/growth/decline claims ("costs explode", "left behind", "grows fast") | payload.chart {kind: bar_up\|bar_down\|line_up\|line_down, label ≤18 chars naming the thing, e.g. "COSTS", "YOUR VALUE"} — synthetic animated chart, no data needed |
| network_grow | scale / systems / networks / teams / many-agents / infrastructure / "at scale" / things connecting or multiplying | payload.network {label ≤18, e.g. "SCALE", "YOUR AGENTS"} — animated node network builds up live; THE go-to for scale talk |
...
12. **THE VISUAL MUST BE SPOKEN (hard — a validator matches tokens):** every photo_cutout / screenshot / logo request must reference a word actually SPOKEN in its beat. **Real photo cutouts are the MOST WANTED visual** — request one whenever ANY concrete noun is spoken: professions ("engineer" → engineer at desk png), objects ("laptop", "code" → mechanical keyboard/code screen png), places, tools, animals, products, people. What is BANNED is unspoken metaphor-scraping (a bull for "bankruptcy", a handshake for "trust") and literal images for pure abstractions (ideas, qualities, feelings — those get Remotion-native treatments or kinetic type).
13. **RICHNESS QUOTA (hard — a validator counts):** at least 40% of all beats must carry a visual element. **Preference order (follow it):** ① photo_cutout of a spoken noun (real PNG imagery — what viral creators use) ② the Remotion-native graphics: network_grow (scale/systems), chart_pop (trends/costs), stat_slam (spoken numbers), list_stack (enumerations), quote_card (thesis lines), tweet_card ③ screenshot_zoom / logo_versus for named products ④ emoji_burst LAST — only when the beat is a raw emotion and nothing concrete is spoken. A manifest of mostly-bare kinetic_type beats WILL BE REJECTED. b01 (hook) and the final beat MUST be visual.
13b. **VARIETY (hard — a validator counts):** emoji_burst may carry AT MOST A QUARTER of the visual beats — emojis read as cheap in short-form; real images and animated graphics read as pro. Named product/site/app spoken → screenshot_zoom or logo. Two named things compared → logo_versus. "Scale"/systems talk → network_grow.
13c. **LOGO AMBIGUITY (hard):** request type=logo ONLY for globally famous tech brands whose logo is unmistakable (GitHub, OpenAI, Google, Apple, Microsoft, Meta, Tesla, Stripe, Amazon…). For niche tools, personal projects, or names that collide with famous brands in OTHER industries (e.g. "Hermes" = a fashion house, "OpenClaw" = unknown) DO NOT request a logo — a wrong-brand logo is worse than none. Use emoji_burst (🤖/⚙️) or kinetic_type instead, or a 3d_icon of the concept.
