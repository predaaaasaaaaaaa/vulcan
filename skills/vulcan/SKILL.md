...
logic lives in one CLI at `{{VULCAN_ROOT}}/bin/vulcan`. Your entire job:
...
```bash
{{VULCAN_ROOT}}/bin/vulcan run --latest          # forge the newest UNCONSUMED cached voice note (the normal trigger path)
{{VULCAN_ROOT}}/bin/vulcan run /path/to/voice.ogg # forge a specific file (testing; also mp3/m4a/wav)
{{VULCAN_ROOT}}/bin/vulcan status <RUN_ID>        # which stage artifacts exist for a run
{{VULCAN_ROOT}}/bin/vulcan cleanup <RUN_ID>       # after user APPROVES the video: reclaim ~70% of the run's disk (keeps final.mp4 + receipts)
{{VULCAN_ROOT}}/bin/vulcan cleanup <RUN_ID> --purge  # user took the file & wants it gone: delete the entire run dir
{{VULCAN_ROOT}}/bin/vulcan cleanup --all          # housekeeping: clean every past run (never touches runs/golden)
{{VULCAN_ROOT}}/bin/vulcan doctor                 # environment self-check (run if anything smells broken)
```
...
under `{{VULCAN_ROOT}}/inbox/` and pass that path. To redo the SAME note:
...
nohup {{VULCAN_ROOT}}/bin/vulcan run --latest > /tmp/vulcan_run.log 2>&1 &
...
**ALWAYS use `{{VULCAN_ROOT}}/bin/vulcan`** — never `python3 -m vulcan.cli`
...
| `DELIVER MEDIA:<path>` | `DELIVER MEDIA:{{VULCAN_ROOT}}/runs/v_…/out/final.mp4` | send a message whose text contains exactly `MEDIA:<that path>` → platform delivers the native video |
...
> **You:** message 1: `MEDIA:{{VULCAN_ROOT}}/runs/v_20260712_1234/out/final.mp4`
...
- Never delete anything under `{{VULCAN_ROOT}}` by hand — cleanup is the
...
cache dirs go in `{{VULCAN_ROOT}}/config.yaml → trigger.voice_cache_dirs`
...
platform-agnostic. Full integration guide: `{{VULCAN_ROOT}}/README.md` §6.

## 9. Hard rules

- ONE run at a time; never edit files under `{{VULCAN_ROOT}}` (invoke the CLI only).
