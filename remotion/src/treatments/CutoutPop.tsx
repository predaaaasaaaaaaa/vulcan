...
const HeroCutout: React.FC<{
  ...
  const {visible, enter, exit} = useAssetWindow(ref, tokens, 'pop');
  const asset = assets[ref.asset_id];
  if (!asset || !visible) return null;

  const isHero = ref.role === 'hero';
  const n = beat.assets.length;
  const tilt = (isHero ? tokens.cutout.rotateDeg : 6) + seeded(beat.id, refIndex * 13) * 4 - 2;
    ...
    <>
      ...
      <div
        style={{
          ...
          left: `${xPct}%`,
          top: `${yPct}%`,
          transform: `translate(-50%, -50%) rotate(${tilt * (0.5 + enter * 0.5)}deg) scale(${scale})`,
